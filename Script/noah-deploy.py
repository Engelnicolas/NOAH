...
class SystemChecker:
    """System requirements and prerequisites checker."""
    ...
    def check_system_requirements(self) -> bool:
        ...
        if cpu_count < self.min_cpu_cores:
            self.logger.error(f"Insufficient CPU cores: {cpu_count} < {self.min_cpu_cores}")
            return False
        ...
        try:
            ...
            if mem_gb < self.min_memory_gb:
                self.logger.error(f"Insufficient memory: {mem_gb:.1f}GB < {self.min_memory_gb}GB")
                return False
            self.logger.info(f"Memory: {mem_gb:.1f}GB ✓")
        except Exception as e:
            self.logger.warning(f"Could not check memory: {str(e)}")
        
        # Check disk space
        try:
            disk_usage = shutil.disk_usage('/')
            disk_gb = disk_usage.free / (1024**3)  # Convert bytes to GB

            if disk_gb < self.min_disk_gb:
                self.logger.error(f"Insufficient disk space: {disk_gb:.1f}GB < {self.min_disk_gb}GB")
                return False
            self.logger.info(f"Disk space: {disk_gb:.1f}GB ✓")
        except Exception as e:
    ...
    def check_prerequisites(self) -> bool:
        ...
        self.logger.info("Checking technical prerequisites...")
        
        required_tools = ['kubectl', 'helm', 'docker', 'minikube', 'ansible']
        missing_tools = []
        
        for tool in required_tools:
            if not shutil.which(tool):
                missing_tools.append(tool)
                self.logger.error(f"Missing required tool: {tool}")
            else:
                self.logger.info(f"Found {tool} ✓")
        
        if missing_tools:
            self.logger.error(f"Missing tools: {', '.join(missing_tools)}")
            self.logger.info("Please install missing tools before proceeding")
            return False
...
class HelmDeployer:
    ...
    def __init__(self, logger: NoahLogger, namespace: str = "noah", timeout: str = "600s"):
        self.logger = logger
        ...
        self.deployment_order = self.priority_charts + self.core_services + self.monitoring_security
    ...
    def deploy_chart(self, chart_name: str, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return True
            error_details = (f"Command: {' '.join(helm_cmd)}\n"
    ...
    def deploy_all_charts(self, dry_run: bool = False) -> bool:
        ...
        for phase_name, charts in phases:
            ...
            for chart_name in charts:
                if self.deploy_chart(chart_name, dry_run):
                    ...
                    self.logger.error(f"Deployment failed for {chart_name} in phase '{phase_name}'")
                    # Continue with other charts in the phase rather than stopping completely
                    continue
            
            self.logger.info(f"Phase '{phase_name}' completed: {phase_success}/{len(charts)} charts deployed")
        ...
        self.logger.info(f"Total deployment: {total_success}/{total_charts} charts deployed successfully")
        
        # Show detailed breakdown
        self.logger.info("Deployment breakdown:")
        for phase_name, charts in phases:
            deployed = sum(1 for chart in charts if chart in self.deployment_order[:total_success])
    ...
    def deploy_priority_charts_only(self, dry_run: bool = False) -> bool:
        ...
        self.logger.info(f"Priority deployment summary: {success_count}/{total_charts} charts deployed successfully")
...
class NoahDeployer:
    ...
    def __init__(self, namespace: str = "noah", timeout: str = "600s", verbose: bool = False, priority_only: bool = False):
        self.namespace = namespace
        self.timeout = timeout
        self.verbose = verbose
    ...
    def run_deployment(self, dry_run: bool = False) -> bool:
        ...
        if not self.system_checker.check_system_requirements():
        ...
        if not self.system_checker.check_kubernetes_cluster():
...
Chart Deployment Phases:
  ...
  3. Monitoring & Security: prometheus, grafana, wazuh, openedr
    ...
    parser.add_argument(
        '--version',
        action='version',
        version='NOAH Deployment Script v1.0.1'
    )
    ...
    if args.list_charts:
        ...
        print(f"  sudo python3 {sys.argv[0]} --priority-only  # Deploy only priority charts")
        print(f"  sudo python3 {sys.argv[0]} --dry-run        # Show what would be deployed")
