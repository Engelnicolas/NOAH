...
{Colors.PURPLE}         Linting Validator            {Colors.NC}
    ...
    def check_prerequisites(self) -> bool:
        ...
        for tool, message in required_tools.items():
            if not shutil.which(tool):
                if tool == "docker":
                    self.print_warning(f"{message} (Super-Linter will not work locally)")
                else:
        ...
        if not (self.project_root / ".pre-commit-config.yaml").exists():
            self.print_error("This script must be run from the NOAH project root directory")
            all_good = False
    ...
    def setup_environment(self) -> bool:
        ...
        try:
            ...
            if not shutil.which("pre-commit"):
                self.print_warning("pre-commit not found in PATH. Adding ~/.local/bin to PATH")
    ...
    def run_pre_commit(self, all_files: bool = False, hook_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.print_warning("Some pre-commit hooks failed. Check the output above.")
                return False
    ...
    def generate_report(self) -> Dict:
        ...
        report["checks"]["pre_commit_installed"] = shutil.which("pre-commit") is not None
...
def main():
    ...
    parser.add_argument(
        "--hook-id",
        help="Specific pre-commit hook to run"
    )
