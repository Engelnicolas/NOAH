...
class NoahMonitoring:
    ...
    def __init__(self, environment: str = "dev", namespace: str = "noah-monitoring", 
...
{Colors.PURPLE}       Monitoring Management         {Colors.NC}
    ...
    def get_helm_releases(self) -> List[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                releases = [line.strip() for line in result.stdout.split('\n') if line.strip()]
                return releases
            else:
                return []
    ...
    def check_monitoring_status(self) -> Dict:
        ...
        status_report["services"] = {"exists": services_exists, "output": services_output}
        
        # Get ingresses
        ingresses_exists, ingresses_output = self.get_namespace_resources("ingresses")
        status_report["ingresses"] = {"exists": ingresses_exists, "output": ingresses_output}
    ...
    def save_status_report(self, status_report: Dict, filename: str = "monitoring_status.json"):
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["deploy", "status", "teardown"],
