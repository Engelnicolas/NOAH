...
def _get_or_generate_secrets(
    ...
    # Garage S3 consumption keys. Generated by _service_generators() in the
    # Garage shape (§6.2) and IMPOSED on Garage afterwards by
    # `noah garage provision`, never the other way round.
    ...
    # One pair of placeholders per Garage consumer. garage-gitmirror is absent
    # ON PURPOSE — it has no consuming namespace and no manifest (§7).
    for service in GARAGE_S3_SERVICES:
        creds = store.get_service_secrets(service)
        suffix = service.removeprefix("garage-").upper()
        replacements[f"REPLACE_WITH_GARAGE_{suffix}_KEY_ID"] = creds.get("access_key_id", "")
        replacements[f"REPLACE_WITH_GARAGE_{suffix}_SECRET"] = creds.get("secret_access_key", "")
...
stringData:
  ...
  OIDC_CLIENT_SECRET: REPLACE_WITH_OIDC_CLIENT_SECRET
""",
    # --- Garage S3 credentials (Garage.md §7) -----------------------------
    #
    # Four Secrets, one per CONSUMING namespace. There is deliberately none for
    # `garage-gitmirror`: the mirror is produced by the backup tooling from the
    # operator workstation, not by a pod (§7), so its key stays in the store
    # and no manifest is rendered for it.
    #
    # These credentials ARE readable from the compute node, by design — being
    # read by the cluster is their purpose. Their compromise is an accepted
    # risk, covered by the ZFS snapshots of §10 of the state map, precisely
    # because Garage cannot grant write without granting delete.
    "apps-extra/nextcloud/garage-s3-secret.enc.yaml": """\
apiVersion: v1
kind: Secret
metadata:
  name: garage-nextcloud-s3
  namespace: nextcloud
type: Opaque
stringData:
  # Env-format keys: the Nextcloud chart consumes primary object storage
  # through OBJECTSTORE_S3_* environment variables.
  OBJECTSTORE_S3_BUCKET: nextcloud-objects
  OBJECTSTORE_S3_HOST: s3.${DOMAIN}
  OBJECTSTORE_S3_PORT: "443"
  OBJECTSTORE_S3_SSL: "true"
  OBJECTSTORE_S3_REGION: garage
  # Garage addresses buckets by path, not by virtual host: the wildcard DNS a
  # virtual-hosted style would need does not exist on this segment.
  OBJECTSTORE_S3_USEPATH_STYLE: "true"
  OBJECTSTORE_S3_KEY: REPLACE_WITH_GARAGE_NEXTCLOUD_KEY_ID
  OBJECTSTORE_S3_SECRET: REPLACE_WITH_GARAGE_NEXTCLOUD_SECRET
""",
    "apps-extra/cnpg/garage-s3-secret.enc.yaml": """\
apiVersion: v1
kind: Secret
metadata:
  name: garage-pgwal-s3
  namespace: cnpg-system
type: Opaque
stringData:
  # Key names follow the CloudNativePG barmanObjectStore convention; the
  # Cluster resource that consumes them arrives with lot 4.
  ACCESS_KEY_ID: REPLACE_WITH_GARAGE_PGWAL_KEY_ID
  ACCESS_SECRET_KEY: REPLACE_WITH_GARAGE_PGWAL_SECRET
  ENDPOINT_URL: https://s3.${DOMAIN}
  BUCKET_NAME: pg-wal
  REGION: garage
""",
    "apps-extra/velero/garage-s3-secret.enc.yaml": """\
apiVersion: v1
kind: Secret
metadata:
  name: garage-velero-s3
  namespace: velero
type: Opaque
stringData:
  # Velero reads an INI credentials profile from a single key called `cloud`.
  cloud: |
    [default]
    aws_access_key_id=REPLACE_WITH_GARAGE_VELERO_KEY_ID
    aws_secret_access_key=REPLACE_WITH_GARAGE_VELERO_SECRET
  BUCKET_NAME: velero
  ENDPOINT_URL: https://s3.${DOMAIN}
  REGION: garage
""",
    "apps-extra/observability/garage-s3-secret.enc.yaml": """\
apiVersion: v1
kind: Secret
metadata:
  name: garage-logs-s3
...
type: Opaque
stringData:
  # Generic S3 environment variables: the log store that consumes them
  # (VictoriaLogs or Loki) arrives with lot 13.
  AWS_ACCESS_KEY_ID: REPLACE_WITH_GARAGE_LOGS_KEY_ID
  AWS_SECRET_ACCESS_KEY: REPLACE_WITH_GARAGE_LOGS_SECRET
...
# THIS LIST MUST STAY IN STEP WITH TWO OTHERS: the namespace loop of the
# `app-secrets` Ansible role, and the Garage flow matrix of cilium_sso.md §7.2.
# Tests/test_garage.py ties the first two together — before that test, nothing
# related them, and a Secret delivered without a matching egress rule (or the
# reverse) only shows up at the first real use of the bucket.
_SECRET_NAMESPACES = (
    "external-dns", "cert-manager", "authentik", "headlamp", "nextcloud", "stalwart",
    # Garage S3 consumers (§7). Pre-created here because the consuming
    # workloads land in later lots — 4 for CNPG, 8 for Velero, 13 for the log
    # store — while the credentials are delivered by this one.
    "cnpg-system", "velero", "observability",
)
