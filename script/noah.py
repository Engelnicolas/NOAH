...
def check_and_install_dependencies():
    ...
    try:
        ...
        print(
            f"{Colors.BLUE}[INFO] Installation de pip: python3 -m ensurepip --upgrade{Colors.NC}"
        )
        try:
            ...
            print(
                f"{Colors.RED}[ERROR] Impossible d'installer pip automatiquement{Colors.NC}"
            )
    ...
    if missing_core:
        print(
            f"{
                Colors.YELLOW}[WARNING] Modules Python manquants: {
                ', '.join(missing_core)}{
                Colors.NC}"
        )
        print(
            f"{Colors.BLUE}[INFO] Veuillez installer python3-full: sudo apt install python3-full python3-venv{Colors.NC}"
        )
    ...
    if missing_packages:
        print(
            f"{
                Colors.YELLOW}[WARNING] Packages Python manquants: {
                ', '.join(
                    [
                        p.split('>=')[0] for p in missing_packages])}{
                Colors.NC}"
        )
        ...
        if not (
            ...
            if venv_path.exists():
                print(
                    f"{Colors.BLUE}[INFO] Environnement virtuel détecté: {venv_path}{Colors.NC}"
                )
                print(
                    f"{Colors.BLUE}[INFO] Activez l'environnement virtuel: source {venv_path}/bin/activate{Colors.NC}"
                )
                print(
                    f"{Colors.BLUE}[INFO] Puis relancez: ./noah <commande>{Colors.NC}"
                )
                return False
            else:
                print(
                    f"{Colors.BLUE}[INFO] Environnement géré exterieurement détecté{Colors.NC}"
                )
                print(
                    f"{Colors.BLUE}[INFO] Utilisez le script wrapper: ./noah <commande>{Colors.NC}"
                )
                print(
                    f"{Colors.BLUE}[INFO] Il créera automatiquement l'environnement virtuel{Colors.NC}"
                )
                return False
        ...
        for package in missing_packages:
            try:
                ...
                print(
                    f"{Colors.BLUE}[INFO] Installation de {package_name}...{Colors.NC}"
                )
                ...
                if result.returncode == 0:
                    print(
                        f"{Colors.GREEN}[SUCCESS] {package_name} installé avec succès{Colors.NC}"
                    )
                else:
                    # Try with --user flag if regular install fails
                    if "externally-managed-environment" in result.stderr:
                        print(
                            f"{Colors.YELLOW}[WARNING] Environnement géré exterieurement{Colors.NC}"
                        )
                        print(
                            f"{Colors.BLUE}[INFO] Utilisez l'environnement virtuel pour installer les dépendances{Colors.NC}"
                        )
                        all_installed = False
                        continue

                    print(
                        f"{Colors.YELLOW}[WARNING] Échec d'installation standard, tentative avec --user{Colors.NC}"
                    )
                    ...
                    if result_user.returncode == 0:
                        print(
                            f"{Colors.GREEN}[SUCCESS] {package_name} installé avec --user{Colors.NC}"
                        )
                    else:
                        print(
                            f"{Colors.RED}[ERROR] Échec de l'installation de {package_name}{Colors.NC}"
                        )
                        print(
                            f"{Colors.BLUE}[INFO] Essayez manuellement: pip install {package_name}{Colors.NC}"
                        )
                        all_installed = False
            except Exception as e:
                print(
                    f"{Colors.RED}[ERROR] Erreur lors de l'installation de {package}: {e}{Colors.NC}"
                )
                all_installed = False
...
def install_requirements_if_missing():
    ...
    if not requirements_file.exists():
        print(
            f"{Colors.YELLOW}[WARNING] Fichier requirements.txt non trouvé{Colors.NC}"
        )
        return False
    ...
    if deps_manager.exists() and os.access(deps_manager, os.X_OK):
        try:
            ...
            if result.returncode == 0:
                print(
                    f"{Colors.GREEN}[SUCCESS] Toutes les dépendances sont installées{Colors.NC}"
                )
                return True
            else:
                print(
    ...
    try:
        print(
            f"{Colors.BLUE}[INFO] Installation des dépendances depuis requirements.txt...{Colors.NC}"
        )
        result = subprocess.run(
        ...
        if result.returncode == 0:
            print(
                f"{Colors.GREEN}[SUCCESS] Dépendances installées avec succès{Colors.NC}"
            )
            return True
        else:
            print(
                f"{Colors.RED}[ERROR] Échec de l'installation des dépendances: {result.stderr}{Colors.NC}"
            )
            return False
    except Exception as e:
...
class NoahCLI:
    ...
    def __init__(self):
        ...
        if not deps_ok:
            print(
                f"{Colors.YELLOW}[WARNING] Certaines dépendances manquent, tentative d'installation...{Colors.NC}"
            )
            install_requirements_if_missing()
        elif has_issues:
            print(
                f"{Colors.GREEN}[SUCCESS] Toutes les dépendances sont disponibles{Colors.NC}"
            )
    ...
    def _check_virtual_environment(self) -> None:
        ...
        if not in_venv:
            print(
                f"{Colors.YELLOW}[WARNING] Vous n'êtes pas dans un environnement virtuel Python{Colors.NC}"
            )
            ...
            if venv_path.exists():
                ...
                print(
                    f"{Colors.BLUE}[INFO] Pour l'activer: source {venv_path}/bin/activate{Colors.NC}"
                )
            else:
                print(
                    f"{Colors.BLUE}[INFO] Il est recommandé d'utiliser un environnement virtuel{Colors.NC}"
                )
                print(
                    f"{Colors.BLUE}[INFO] Le script ./noah créera automatiquement l'environnement{Colors.NC}"
                )
        else:
            venv_path = os.environ.get('VIRTUAL_ENV', 'unknown')
            print(
                f"{Colors.GREEN}[SUCCESS] Environnement virtuel actif: {venv_path}{Colors.NC}"
            )
            ...
            for script_path in tests_dir.glob("noah-*.py"):
                if script_path.is_file():
                    ...
                    commands[command_name] = {
                        ...
                        "requires_root": self._check_requires_root(script_path),
                        "requires_subcommand": self._check_requires_subcommand(
                            script_path
                        ),
                        "subcommands": self._get_script_subcommands(script_path),
                    }
            ...
            if (
                ...
                if command_name not in commands:  # Don't override Python scripts
                    ...
                    commands[command_name] = {
                        "script_path": script_path,
                        "execution_method": (
                            "python" if is_python_script else "executable"
                        ),
                        "description": f"Script exécutable: {
                            script_path.name}",
                        "requires_root": self._check_requires_root(script_path),
                        "requires_subcommand": (
                            self._check_requires_subcommand(script_path)
                            if is_python_script
                            else False
                        ),
                        "subcommands": (
                            self._get_script_subcommands(script_path)
                            if is_python_script
                            else []
                        ),
                    }
        ...
        try:
            if script_path.suffix == ".py":
                with open(script_path, 'r', encoding='utf-8') as f:
                    ...
                    if (
                        'choices=' in content and 'argparse' in content
                    ) or 'add_subparsers' in content:
                        return True
        except Exception:
            pass
        return False
...
{Colors.NC}"""
    ...
    def check_root_privileges(self, command: str) -> bool:
        ...
        self.print_info(
            "Les opérations suivantes nécessitent des privilèges administrateur :"
        )
        ...
        response = input(
            f"{Colors.YELLOW}Voulez-vous continuer avec sudo ? [y/N]{Colors.NC} "
        )
    ...
    def execute_script(self, command: str, args: List[str]) -> int:
        ...
        if script_info.get("requires_subcommand", False) and not args:
            ...
            if command == "linter":
                ...
                if not setup_status:
                    self.print_warning(
                        "L'environnement de linting n'a pas encore été configuré"
                    )
                    self.print_info(
                        "Voulez-vous exécuter la configuration maintenant ?"
                    )
                    response = input(
                        f"{Colors.YELLOW}Exécuter 'noah linter setup' ? [y/N]{Colors.NC} "
                    )
                    if response.lower() in ['y', 'yes', 'o', 'oui']:
            ...
            if subcommands:
                ...
                if command == "linter":
                    self.print_info("Sous-commandes disponibles:")
                    self.print_info(
                        "  • setup     - Configurer l'environnement de linting"
                    )
                    self.print_info(
                        "  • lint      - Exécuter le linting sur les fichiers"
                    )
    ...
    def show_help(self) -> None:
        ...
        print(
            f"    ./noah --list                           # Liste détaillée des scripts"
        )
        ...
        for category, commands in categories.items():
            available_commands = [
                cmd for cmd in commands if cmd in self.script_commands
            ]
                ...
                for cmd in available_commands:
                    ...
                    print(
                        f"  {
                            ...
                            cmd:<15}{
                            Colors.NC} {
                            script_info['description']}{root_indicator}"
                    )
                print()
        ...
        other_commands = [
            cmd for cmd in self.script_commands.keys() if cmd not in shown_commands
        ]
        if other_commands:
        ...
        print(
            "    ./noah linter setup                     # Configurer l'environnement"
        )
        print(
            "    ./noah linter lint --all                # Linting sur tous les fichiers"
        )
    ...
    def list_scripts(self) -> None:
        ...
        for cmd, info in sorted(self.script_commands.items()):
            ...
            method_color = (
                Colors.GREEN if execution_method == "python" else Colors.YELLOW
            )
            ...
            print(
                f"    └─ {method_color}{
                    script_path.name}{
                    Colors.NC} ({execution_method}){root_indicator}"
            )
            print()
